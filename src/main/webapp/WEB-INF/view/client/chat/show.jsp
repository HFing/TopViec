        ...
        <div class="row chat-columns">
            <div class="col-md-4 column users-list">
                ...
                <ul class="list-group">
                    <c:forEach var="user" items="${users}">
                        <li class="list-group-item d-flex align-items-center" onclick="loadChatHistory('${user}')">
                            <img src="/images/avatar/default_avatar.jpg" alt="${user} Avatar" class="rounded-circle">${user}
                        </li>
                    </c:forEach>
                </ul>
            </div>
            <div class="col-md-4 column chat-box">
                ...
                <div class="chat-box-header">
                    <img src="https://zos.alipayobjects.com/rmsportal/ODTLcjxAfvqbxHnVXCYX.png" alt="Sender Avatar" class="avatar rounded-circle">
                ...
                <div class="input-group mb-3">
                    <input type="text" placeholder="Type a message..." class="form-control chat-input">
                    <div class="input-group-append">
                        <button class="btn btn-primary send-button" type="button" onclick="sendMessage()">Send</button>
                    </div>
                </div>
            </div>
                ...
                <ul class="list-group">
                    <c:if test="${not empty appliedCompanies}">
                        <c:forEach var="company" items="${appliedCompanies}">
                            <li class="list-group-item d-flex align-items-center" style="box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                                <img src="/images/company/${company.companyImageUrl}" alt="${company.companyImageUrl} Avatar" class="rounded-circle">
                                ${company.user.fullName} - Company: ${company.companyName}
                                <button class="btn btn-primary ml-auto" onclick="selectRecipient('${company.user.fullName}', '${company.companyImageUrl}')">Nhắn tin</button>
                            </li>
                        </c:forEach>
                    </c:if>
                    <c:if test="${empty appliedCompanies and not empty applicants}">
                        <c:forEach var="applicant" items="${applicants}">
                            <li class="list-group-item d-flex align-items-center" style="box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); margin-top: 10px;">
                                <img src="/images/avatar/${applicant.user.avatarUrl}" alt="${applicant.user.fullName} Avatar" class="rounded-circle">
                                ${applicant.user.fullName}
                                <button class="btn btn-primary ml-auto" onclick="selectRecipient('${applicant.user.fullName}', '${applicant.user.avatarUrl}')">Nhắn tin</button>
                            </li>
                        </c:forEach>
                    </c:if>
                    <c:if test="${empty appliedCompanies and empty applicants}">
                        <li class="list-group-item" style="box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">No companies or applicants available.</li>
                    </c:if>
                </ul>
            </div>
        </div>
